...
demo_show_setup:
	...
	docker run -e "TERM=xterm-256color" -v $(shell pwd):/local $(IMG_NAME):$(IMG_VERSION) ansible-playbook setup_nautobot.yml --limit rtr-1

.PHONY: demo_setup
demo_setup:
	@echo "Starting the Setup demo"
	ansible-playbook setup_nautobot.yml --tags ios

.PHONY: demo_audit
demo_audit:
	@echo "Starting Audit of single device"
	ansible-playbook audit_nautobot.yml --limit wanrtr01
	read
	ansible-playbook audit_nautobot.yml
